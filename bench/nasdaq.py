...
DATA_DIR = Path("data")
NASDAQ_CSV = DATA_DIR / "nasdaq_daily.csv"

def _fetch_qqq_yfinance():
    """
    Free daily QQQ prices (USD) from Yahoo Finance (proxy for Nasdaq 100).
    """
...
def get_nasdaq_daily(start: str, end: str, cache_path: Path = NASDAQ_CSV):
    """
    DAILY (business-day) Nasdaq 100 proxy in GBP with stable returns.
    ...
    # 2) QQQ (USD) from Yahoo Finance
    qqq = _fetch_qqq_yfinance().copy()
    qqq["date"] = pd.to_datetime(qqq["date"]).dt.date
    qqq = (qqq.set_index("date")
              .reindex(cal)
              .ffill()
              .bfill())
    qqq["close_usd"] = pd.to_numeric(qqq["close_usd"], errors="coerce").astype("float64")
    ...
    df["close_usd"] = qqq["close_usd"].astype("float64")
